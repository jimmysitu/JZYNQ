...
void main(){
    ...
    *dev = open(TTYDEVICE, O_RDWR | O_NOCTTY | O_NDELAY | O_NONBLOCK);
    if(*dev == -1){
        ...
        printf("Opened tty device %d\n", *dev);
    }

    if(-1 == ioctl(*dev, TIOCEXCL)){
        perror("Fail to lock tty device:");
    }else{
        printf("Locked tty device\n");
    }
...
//    }
    ...
    //fcntl(*dev, F_SETFL, 0 | O_NONBLOCK);
...
//    if(-1 == flock(*dev, LOCK_EX)){
//        perror("Fail to unlock tty device:");
//    }else{
//        printf("Unlocked tty device\n");
//    }
